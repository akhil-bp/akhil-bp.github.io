...
(function (factory) {
  ...
  EvoCalendar.prototype.buildTheBones = function () {
    ...
    if (!_.$elements.calendarEl.html()) {
      ...
      markup +=
        ...
        '<div class="total-events"><span></span></div>' +
  ...
  EvoCalendar.prototype.buildEventList = function () {
    ...
    const date = new Date(_.$active.date);
    const month = new Intl.DateTimeFormat("en", { month: "long" }).format(date);

    var title = `EVENT FOR ${month.toUpperCase()}`;
    ...
    if (_.options.calendarEvents) {
      ...
      _.$elements.eventEl
        .find(".total-events > span")
        .text(_.$active.events.length);
    }
  ...
  EvoCalendar.prototype.addEventList = function (event_data) {
    ...
    const d = new Date(event_data.date);
    const selectedDate = new Date(this.$active.date);
    const ye = new Intl.DateTimeFormat("en", { year: "numeric" }).format(
      selectedDate // Recurring event has sepecific date like 1995
    );

    const mo = new Intl.DateTimeFormat("en", { month: "2-digit" }).format(d);
    const da = new Intl.DateTimeFormat("en", { day: "2-digit" }).format(d);
    const t = new Intl.DateTimeFormat("en", { timeStyle: "short" }).format(d);
    markup += `<p class="event-desc">${da}-${mo}-${ye} ${t}</p>`;
    markup +=
      "<div style='margin-top:10px'><button class='btn-live live red'>LIVE<button> <button class='btn-event-secondary'>Join<button></div>";
    markup += "</div>";
    markup += "</div>";
    eventListEl.append(markup);
